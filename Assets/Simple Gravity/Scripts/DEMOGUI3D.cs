...
public class DEMOGUI3D : MonoBehaviour {
		...
		if(!hideBallOptions)
		{
			if (objectsDrag != oldObjectsDrag)
			{
				foreach(GameObject a in goList)
				{
					...
					{
						a.GetComponent<Rigidbody>().drag = objectsDrag;
					}
				}
			...
			if (gravity != oldGravity)
			{
				foreach(GameObject a in goList)
				{
					if (a.GetComponent<Rigidbody>() != null)
					{
						a.GetComponent<Rigidbody>().useGravity = gravity;
					}
				}
				oldGravity = gravity;
			}
	...
	void OnGUI()
	{
		if (gravityComponent != null)
		{
			...
			gravityComponent.strength = GUILayout.HorizontalSlider(gravityComponent.strength,0f,500f,GUILayout.Width(175));
